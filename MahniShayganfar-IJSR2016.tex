...
appraisal processes as part of a new overall computational model. we have also
developed a goal management algorithm based on a cost function that we use to
choose the goal in the shared plan during collaboration. We have evaluated our
implemented appraisal algorithms by conducting an online user study.
...
In this section, we introduce our goal management process based on a cost
function including the influence of affective appraisal and reverse appraisal
processes. Goal management is a crucial part of our investigation of the
reciprocal influence of appraisal on a collaboration structure (see Fig.
\ref{fig:goalManagement}).
...
\begin{figure}[tbh]
  ...
  \label{fig:goalManagement}
  \vspace*{-2mm}
\end{figure}

Appraisals are separable antecedents of emotion with which the robot
evaluates the environment. The outcome of each appraisal process is a specific
value for the corresponding appraisal variable (see Section
\ref{sec:appraisal}). The vector containing these appraisal variables can be
...
goals in the form of a shared plan (see Fig. \ref{fig:cs-goalManagement})
which contains both the robot and the human collaborator's goals. The robot
pursues the goals for which the robot is responsible in the shared plan.
However, there can be several live goals available for the robot to pursue at
each point in time during collaboration. A goal is \textit{live} if all of its
...
For instance, Fig. \ref{fig:cs-goalManagement} shows a non-primitive ``Prepare
Panels'' goal decomposed into three primitive goals. Therefore, if ``Prepare
Panels'' is live, its primitive goals can be pursued by the responsible agent.
In our example, the astronaut is responsible for the ``Check Connector'' goal;
the robot is responsible for the remaining two primitive goals. According to the
...
$S(g)$ as three attributes of a given goal (see equations \ref{eqn:proximity} to
\ref{eqn:specificity}).
...
Further studies are required to obtain more appropriate values of these weights.
...
\ref{fig:cs-goalManagement}, the values of $S(g)$ for the three primitives under
the ``Prepare Panels'' are 2.
...
The tuples below the goals in Fig. \ref{fig:cs-goalManagement} indicate the cost
value of each goal. The first number in each tuple is the normalized cost value
...
Connector'' is 0.82 (see Fig. \ref{fig:cs-goalManagement}). As shown, when
